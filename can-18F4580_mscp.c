 ...
 *
 * This library provides the following functions: (for more information on these
 *  functions see the comment header above each function)
 *
 ...
 *
 *    can_- Returns true if there is data in one of the receive buffers**
 *
 ...
 *
 *    can_associate_filter_to_buffer - associates a filter with a receive buffer
 *
 *    can_associate_filter_to_mask - associates a filter with a mask
 *
 *    can_fifo_getd - retrive data in FIFO mode (2)
 *
 *    can_t*_putd - place data in a transfer buffer
 *
 *    can_b*_putd - place data in a programmable buffer
 *
 * PIN_B3 is CANRX, and PIN_B2 is CANTX. You will need a CAN transeiver to 
 *  connect these pins to CANH and CANL bus lines.
 *
 ...
 * can_init()
 *
 * Initializes PIC18xxx8 CAN peripheral. Sets the RX filter and masks so the CAN
 * peripheral will receive all incoming IDs. Configures both RX buffers to only
 * accept valid valid messages (as opposed to all messages, or all extended
 * message, or all standard messages). Also sets the tri-state setting of B2 to
 * output, and B3 to input (apparently the CAN peripheral doesn't keep track of
 * this)
 *
 ...
 * can_set_baud()
 *
 * Configures the baud rate control registers. All the defines here are
 * defaulted in the can-18xxx8.h file. These defaults can, and probably should,
 * be overwritten in the main code.
 *
 * Current defaults are set to work with Microchip's MCP250xxx CAN Developers
 * Kit if this PIC is running at 20Mhz.
 *
 *  BRGCON1 contains the prescaler bits and the Synchronization Jump Width time
 *      bits. The prescale values are
 *
 ...
 *      000100=10/clock profided that the user does not define it differently.
 ...
 *
 *  BRGCON2 contains the Phase Segment 2 Time Select bit, the Sample bit, the
 *      Phase Segment 1 bits, and the Propagation Time Select bits
 *
 *          SEG2PHTS
 *              1=Freely Programmable
 *              0=Maximum of PHEG1 or IPT, which ever is greatest
 *
 *      in the case of can-18xxx8.h, the SEG2PHTS bit is set to 1 for freely
 *      programmable
 *
 *          SAM
 *              1=Three Samples
 *              0=One Sample
 *
 *      in the case of can-18xxx8.h, the SAM bit is set to 0 for one sample
 *
 *          SEG1PH2:SEG1PH0
 *              Phase Segment 1 = (SEG1PH2:SEG1PH0+1)*Tq
 *
 *      in the case of can-18xxx8.h, the SEG1PH2:SEG1PH0 bits are set to 5 for
 *      6*Tq Phase Segment 1 Time
 *
 *          PRSEG2:PRSEG0
 *              Propogation Time = (PRSEG2:PRSEG0+1)*TQ
 *
 *      in the case of can-18xxx8.h, the PRSEG2:PRSEG0 bits are set to 2 for
 *      3*Tq Propogation Time
 *
 ...
 *
 *      in the case of can-18xx8.h, the WAKEFIL bit is set to 0 for CAN bus not
 *      used for wake-up
 *
 *          SEG2PH2:SEG2PH0
 *              Phase Segment 2 Time = (SEG2PH2:SEG2PH0+1)*Tq
 *
 *      in the case of can-18xxx8.h, SEG2PH2:SEG3PH0 is set to 5 for 8*Tq Phase
 *      Segment 2 Time
 *
 ...
 * can_set_mode
 *
 * This function first sets three most significant bits (reqop2:reqop0) to the
 * mode given through the argument. The modes can be
 *
 *  mode
 *
 ...
 *              This mode allows the PIC microcontroller to listen to the bus
 *              but not actually write to it.
 *
 *  010 ->  Loop Back Mode
 *              This mode allows to the transmission of data directly from the
 *              Transmission Buffers to the Recive Buffers without actually
 *              driving any data to the bus.
 *
 *  001 ->  Dissable Mode
 *              This mode prefents the PIC microcontroller from transmitting and
 *              receiving, however the WAKIF interrupt is still active.
 *
 *  000 ->  Normal Mode
 *              This is the normal mode of opperation for the CAN bus. It will
 *              recive all messages and is the only mode in which data can be
 *              transmitted.
 *
 * The reqop bits do not immediatly change the mode of operation, the three most
 * significant bits in the CANSTAT register (opmode2:opmode0) must change to
 * reflect the actuall change in mode, therefore a while statement is used to
 * check if the CANSTAT opmode bits have changed to reflect the passed in mode.
 *
 ...
 *  mode
 *
 *  00  ->  mode 0 or Legacy Mode. 
 *              The processor defaults to this mode on reset in order to
 *              backwards compatibility with the previous PIC18Cxx8 and 
 *              PIC18Fxx8 CAN modules. All code written for the listed
 *              microcontrollers will work on the PIC18F4580 with no alterations.
 *
 ...
 *
 *  01  ->  mode 1 or Enhanced Legacy Mode. 
 *              This mode is the same as mode 0 with the exception that there
 *              are more resources available to the CAN module. These include
 *
 *              3  Transmit Buffers: TXB0, TXB1, TXB2
 *              2  Recive Buffers: RXB0, RXB1
 ...
 *
 *  10  ->  mode 2 or Enhanced FIFO Mode. 
 *              This mode produces a first in first out (FIFO) CAN data buffer.
 *              This buffer can be from 2 to 8 bytes in length depending on 
 *              which B buffers are used as recive buffers and which are used as
 *              transmit buffers. The first transmit buffer defines the size,
 *              therefor if B4 was the first transmit buffer, RXB0, BXB1, B0,
 *              B1, B2, and B3 would form the FIFO buffer for a size of 6 bytes.
 *              If B0 through B5 are all set to recive data, the size will be 8
 *              bytes. If B0 is set to transmit, the size will be 2 bytes. 
 *
 ...
 * can_putd()
 *
 * Puts data on a transmit buffer, at which time the CAN peripheral will send
 * when the CAN bus becomes available.
 *
 *  Parameters:
 *      id - ID to transmit data as enumerated as - RXB0ID,RXB1ID,B0ID,B1ID,
                B2ID,B3ID,B4ID,B5ID
 *      data - pointer to data to send
 *      len - length of data to send
 *      priority - priority of message. The higher the number, the sooner the
                CAN peripheral will send the message. Numbers 0:3 are valid.
 ...
 *      stat - structure holding some information (such as which buffer received
 *              it, ext or standard, etc)
 *
 *  Returns:
 ...
 *
 * Sets one of the 6 programmable buffers to be a transfer buffer. This uses a
 * simple bitwise operation so that more than one buffer can be set at a time.
 *
 * The register that contains the TX/RX information is BSEL0 and the control
 * bits take up the 6 most significant bits. Therefore a value of 0x04 would set
 * B0 as a transmit buffer while a value of 0xFC would set all of the
 * programmable buffers to transmit buffers.
 *
 *  Parameters:
 *      b - the buffer which is to become a transfer buffer enumerated as:
 *      B0,B1,B2,B3,B4,B5
 *
 ...
 *
 * Sets one of the 6 programmable buffers to be a receiver buffer. This uses a
 * simple bitwise operation so that more than one buffer can be set at a time.
 *
 * The register that contains the TX/RX information is BSEL0 and the control
 * bits take up the 6 most significant bits. Therefore a value of 0x04 would set
 * B0 as a receive buffer while a value of 0xFC would set all of the
 * programmable buffers to receive buffers.
 *
 *  Parameters:
 *      b - the buffer which is to become a receiver buffer enumerated as:
 *      B0,B1,B2,B3,B4,B5
 *
 * More information can be found on the programmable buffers in the PIC18F4580
 * datasheet, section 23.5.3
 ...
 *
 *  Parameters:
 *      b - the programmable buffer to be set to Auto-rtr mode enumerated as:
 *          B0,B1,B2,B3,B4,B5
 *
 ...
 *
 *  Parameters:
 *      b - The programmable buffer to be filled enumerated as:
 *          B0,B1,B2,B3,B4,B5
 *      data - the address of the first data byte
 *      len - the number of data bytes to be copied
 *
 *   Note: the programmer must set the appropriate rtr registers in order for
 *         the rtr function to work. This function does not set any of those
 ...
 *
 *  Parameters:
 *      filter - the filter that is to be enabled enumerated as RXF0EN-RXF15EN
 *
 ...
 *
 *  Parameters:
 *      filter - the filter that is to be disabled enumerated as RXF0EN-RXF15EN
 *
 * More information can be found on Acceptance Filters in the PIC18F4580
 * datasheet, Section 23.8
 */
 ...
 *
 *  Parameters:
 *      buffer - the buffer to be associated with the filter enumerated as
 *              ARXB0-ARXB1 - Dedicated receive buffers RX0-RX1
 *              AB0-AB5 - Programmable receive buffers B0-B5
 *
 *      filter - the filter to be associated with the buffer enumerated as
 ...
 *
 *  Parameters:
 *      mask - the mask that is to be associated with the filter enumerated as
 *              ACCEPTANCE_MASK_0
 *              ACCEPTANCE_MASK_1
 *              FILTER_15
 *              NO_MASK
 *
 *      filter - the filter that is to be associated with the mask enumerated as
 *              F0BP-F15BP - Filters 0 - 15
 *
 ...
 *
 * Note:
 *      Make sure that the desired buffer is set to be a trasfer buffer using
 *      the can_enable_b_transfer() function
 *
